...
client.on('ready', () => {
    ...
    const lavalink = client.player.join({
        guild: 'your_guild_id',
        node: {
            host: process.env.LAVALINK_URL,
    ...
    lavalink.on('error', (err) => console.error(`Lavalink Node Error: ${err}`));
    lavalink.on('close', (reason) => console.warn(`Lavalink Node Closed: ${reason}`));
});
